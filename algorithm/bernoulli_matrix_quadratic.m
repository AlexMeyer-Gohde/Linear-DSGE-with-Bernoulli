function [X,varargout] = bernoulli_matrix_quadratic(matrix_quadratic,varargin)
...
while diff>tol % As long as the iterations haven't converged...
    ...
    elseif mbi
        ...
        if line_search
            alpha=norm(M,'fro')^2;
            Y_1=delta_X(npred+1:end,:)*delta_X(1:nspred,:);
            Z_1=delta_X(npred+1:end,:)*X(1:nspred,:);
            Y_2=A*Y_1;
            Z_2=A*Z_1;
            if matrix_quadratic.nstatic>0
                Y_1=matrix_quadratic.A_static*Y_1;
                Z_1=matrix_quadratic.A_static*Z_1;
                gamma=trace(Y_1*Y_1')+trace(Y_2*Y_2');
                beta=trace(Z_1*Z_1')+trace(Z_2*Z_2');
                xi=2*trace(Y_1*Z_1')+2*trace(Y_2*Z_2');
            else
                gamma=trace(Y_2*Y_2');
                beta=trace(Z_2*Z_2');
                xi=2*trace(Y_2*Z_2');
...
% 
%             if discr>0+eps %three real roots
%                 if (a+b+c+d)*d>0 && b*(3*a+b)>0%No root or two roots in interval 0, 1 % No root in 0 1
%                     s=1;
%                 else
%                     k=0:2;
%                     root=2*(-p/3)^(1/2)*cos(1/3*acos(3/2*q/p*(-3/p)^(1/2))-2*pi*k/3);
...
%                     end
%                     values=(gamma*root.^4+(xi-sigma)*root.^3+(alpha+beta-delta+sigma)*root.^2+(delta-2*alpha)*root+alpha);
%                     s=root(values==min(values));
%                 end
%             elseif discr<0-eps %one real root
%                 if (a+b+c+d)*d>0 %No root in interval 0, 1
%                     s=1;
%                 else
%                     if p>0
%                         s=-2*(p/3)^(1/2)*sinh(1/3*asinh(3/2*q/p*(3/p)^(1/2)));
%                     else
%                         s=-2*abs(q)/q*(-p/3)^(1/2)*cosh(1/3*acosh(-3*abs(q)/(2*p)*(-3/p)^(1/2)));
%                     end
%                     s=s-b/(3*a);
%                 end
%             else
...
%             value_1=alpha+a/4+b/3+c/2+d;
%             if (value_1-value_s)/value_s<-eps&&(value_1-value_s)<-eps
%                 s=1
%             end
% if s<0
%     disp(s)
...
s=s(end);
    ...
    AX_B=[A*X(npred+1:end,:) zeros(ndynamic,nfwrd)]+B;
    M=AX_B*X+C;
    diff=convergence_criterion(M,X,X_0,AX_B,A,B,C,convergence_metric);           % determine convergence criterion
